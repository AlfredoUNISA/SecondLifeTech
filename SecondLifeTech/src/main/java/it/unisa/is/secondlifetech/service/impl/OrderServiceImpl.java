...
public class OrderServiceImpl implements OrderService {
     ...
     */
    @Override
    public Page<OrderPlaced> findAllOrdersPaginatedWithFilters(OrderFilters filters, Pageable pageable) {
        List<OrderPlaced> allOrders = orderPlacedRepository.findAll();
        List<OrderPlaced> filteredOrders = doFilter(filters, allOrders);
    ...
    /**
     * Esegue il filtraggio degli ordini.
     */
    private List<OrderPlaced> doFilter(OrderFilters filters, List<OrderPlaced> allOrders) {
        List<OrderPlaced> filteredOrders = new ArrayList<>();

        if (filters.getEmail() != null && !filters.getEmail().isEmpty()) {
            // Filtra per email
            filteredOrders.addAll(allOrders.stream()
                    .filter(order -> order.getEmail().equalsIgnoreCase(filters.getEmail()))
                    .toList()
            );
